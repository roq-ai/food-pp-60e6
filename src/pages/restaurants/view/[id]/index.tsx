...
import { deleteFoodItemById } from 'apiSdk/food-items';
...
function RestaurantViewPage() {
  ...
  const food_itemHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('food_item', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/food-items/create?restaurant_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.food_item?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/food-items/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.name}</Td>
                            ...
                            <Td>
                              {hasAccess('food_item', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/food-items/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('food_item', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    food_itemHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
